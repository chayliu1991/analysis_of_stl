...
//@ 第一级配置器
//@ 无「template型别参数」。至于「非型别参数」inst，完全没派上用场
template <int __inst>
class __malloc_alloc_template {
 ...
 public:
  ...
  //@ 以下模拟 c++ 中的set_new_handler(),可以指定自己的oom handler
...
class __default_alloc_template {
 ...
 private:
  ...
  static char* _S_start_free;  //@ 内存池起始位置。只在 chunk_alloc()变化
 ...
 public:
  ...
  static void deallocate(void* __p, size_t __n) {
    ...
    if (__n > (size_t)_MAX_BYTES)
      ...
      //@ 寻找对应的free_list并调整free_list,回收区块
      _Obj* __STL_VOLATILE* __my_free_list =
          _S_free_list + _S_freelist_index(__n);
      _Obj* __q = (_Obj*)__p;
...
char* __default_alloc_template<__threads, __inst>::_S_chunk_alloc(
  ...
  if (__bytes_left >= __total_bytes) {
    ...
    return (__result);
  } else if (__bytes_left >= __size) {       //@ 剩余一个区块以上空间
    __nobjs = (int)(__bytes_left / __size);  //@ 实际可以分配的个数
